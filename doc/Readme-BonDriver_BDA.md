...
#### DD.dll
  ...
  - Digital Devices GmbH社製の各種チューナー

#### DVBWorld.dll

現在バイナリでの配布は行っていません。ソースコードからビルドしてください
...
PCはCPUアーキテクチャやOSがそれぞれ異なります。それに合わせて使用するファイルを選択する必要があります。
現在サポートしている選択肢は下記のとおりです。全部で６種類あります。
...
「[Visual Studio 2017 の Microsoft Visual C++ 再頒布可能パッケージ](https://visualstudio.microsoft.com/ja/downloads/?q=#other-ja)」の最新版を下記URLよりダウンロードし、インストールしてください。
https://visualstudio.microsoft.com/ja/downloads/?q=#other-ja
**※x86/x64の2種類がありますので注意してください。**
よくわからない場合は両方インストールしても大丈夫です。
...
#### BonDriverとiniファイル等の配置

使用するアプリのBonDriver配置フォルダ内に、必要なファイルを配置していきます。

### BonDriver_BDA.dllのコピーを配置

BonDriver_BDA.dllをコピーしてリネームしたものを配置してください。
このとき、ファイル名が`BonDriver_`から始まるようにリネームする必要があります。ご注意ください。

```
./
└─ BonDriver_<お好きな名前>.dll
```
...
また、iniファイルに設定できる各項目の説明は`BonDriver_BDA.sample.ini`を参照してください。
...
※「Bon_SPHD_BDA_PATCH_2」相当のBDASpecialプラグインを使用する場合で、同一のプロセスから複数のBonDriverの読込が行われる場合は下記のようにBDASpecialプラグインも複数用意し、iniファイルで`UseSpecial="DVBWorld_0"`のように個別にファイル名を指定してください。
